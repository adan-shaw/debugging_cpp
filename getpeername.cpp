...
/*
	...
	* 根据'本机的sfd', 获取该sfd的'struct sockaddr_in 地址信息'
	* 成功返回0, 出错返回-1
...
//服务端
void srv(char *bind_ip, unsigned short port);

//客户端
void cli(char *bind_ip, unsigned short port);
...
void srv(char *bind_ip, unsigned short port){
	...
	struct sockaddr_in 
			addr_srv, addr_bind, addr_conn, addr_peer;
...
void cli(char *bind_ip, unsigned short port){
	...
	getpeername(sfd_cli, (struct sockaddr *)&addr_cli, &addr_cli_len);
	printf("cli: getpeername() 提取sfd_cli对端的addr info = %s:%d\n",
			inet_ntop(AF_INET, &addr_srv.sin_addr, str_ip, sizeof(str_ip)),
			ntohs(addr_srv.sin_port));
