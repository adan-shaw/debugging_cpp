...
1.__init__.py 文件的作用是: 
	...
	我们在import 导入一个包时, 实际上是导入了它的__init__.py文件;
	将__init__.py 定义的所有的py 文件, 联合组成一个新的py模块, 供import 导入到python;

	具体引用哪些py 文件, 由setup.py 文件指定;
	具体怎样初始化这个封装包, 怎样初始化这些*.py 文件, 由__init__.py文件指定;

	__init__.py 可以导入内建库包, 也可以导入自定义包;
	...
	__init__.py 主要控制包的导入行为, 要想清楚理解__init__.py文件的作用, 还需要详细了解一下import语句引用机制;
