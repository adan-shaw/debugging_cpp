...
'''
	...
	因此, python 的'变量内存'重用规则如下(根据'左值/右值'的概念进行划分):
		* 左值=变量本身,
			右值='常量/实体常量/函数返回/new 关键字返回/copy拷贝函数返回/+号*号:号等运算符重载', 
			则: 变量本身(即左值), 触发copy拷贝;
	...
	ps2:
		不同类型的容器, 如何进行拷贝?
		答:
			尽量使用系统自带的API 进行拷贝, 性能最好, 例如: 使用list() 拷贝dict 的keys()/values()
			则有:
				# 前提: dict 中的keys/values, 元素类型一致;
				d1={'a':1,'b':2,'c':3,'d':4,'e':5}
				l1_keys=list(d1.keys())
				l1_values=list(d1.values())

			如果元素类型不一致, 才需要用到逻辑拷贝(性能极度不好), 
			但逻辑拷贝优点是: 可以剔除任意元素, 不符合标准的, 就剔除;
'''



print("左值=变量本身, 右值='常量/实体常量/函数返回/new 关键字返回/copy拷贝函数返回/+号*号:号等运算符重载'")
a=(1,2,3,4,5)
b=(1,2,3,4,5)
print(id(a))
print(id(b))
...
print("左值=复杂变量的子元素, 右值='常量/实体常量/函数返回/new 关键字返回/copy拷贝函数返回/+号*号:号等运算符重载'")
a=[1,2]
b=a
print(a,id(a))
print(b,id(b))
