...
int main(void){
	...
	pair<multiset<int>::iterator, multiset<int>::iterator> multiset_pair_range;
	...
	/*
	multiset_pair_range = x1.equal_range(500);											//超出范围的值搜索, 会直接等于end(), 表示被搜索的值不存在
	assert(multiset_pair_range.first == x1.end());

	multiset_pair_range = x1.equal_range(223);											//刚好搜索到最后一个值, multiset_pair_range.second == x1.end()
	assert(multiset_pair_range.second == x1.end());

	assert(multiset_pair_range.first != multiset_pair_range.second);//multiset_pair_range.first 永远不等于 multiset_pair_range.second 
	*/
	multiset_pair_range = x1.equal_range(20);
	if(multiset_pair_range.first != x1.end())
		for(; multiset_pair_range.first != multiset_pair_range.second; multiset_pair_range.first++)
			printf("13 -- x1.equal_range(20)=%d [pair.second=%d]\n", *multiset_pair_range.first, *multiset_pair_range.second);
