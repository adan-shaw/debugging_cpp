//编译:
//		g++ -std=c++11 -g3 ./unordered_map.hash_func.cpp -w -o x 
...
int main(void){
	unordered_map<Customer, float, CustormerHash, CustormerKeyCmp> x1;
	unordered_map<Customer, float, CustormerHash, CustormerKeyCmp>::iterator it;
	//unordered_map<Customer, float, hash_func, key_func> x2;							//失败
	//unordered_map<Customer, float, hash_func, key_func>::iterator it2;	//失败
	Customer m("陈0");
