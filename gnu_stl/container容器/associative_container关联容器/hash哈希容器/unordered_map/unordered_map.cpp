//编译:
//		g++ -std=c++11 -g3 ./unordered_map.cpp -w -o x 



//unordered_map api list
/*
	成员函数
	...
	非成员函数
		operator==					比较 unordered_map 中的值
		operator!=
		std::swap(std::unordered_map)		特化 std::swap 算法(C++11)
		erase_if(std::unordered_map)		擦除所有满足特定判别标准的元素(C++20)
*/
...
int main(void){
	...
	unordered_map<int,int> x2 { {1,1},{2,10},{3,100},{4,1000}, };
	unordered_map<int,int> x3(x2.begin(), x2.end());	//iterator 构造copy
	unordered_map<int,int>::iterator it,itlow,ithigh;	//顺向迭代器
	unordered_map<int,int>::hasher fn;								//hash 函数
	unordered_map<int,int>::key_equal eq;							//key 值比较函数
																										//pair一对变量
	pair<unordered_map<int,int>::iterator, bool> hash_map_pair_val;
	pair<unordered_map<int,int>::iterator, unordered_map<int,int>::iterator> hash_map_pair_range;


	//2.void clear();																	//清空unordered_map
	x1.clear();
	x3.clear();



	//3.bool empty();																	//检查unordered_map 是否为空
	if(x1.empty())
		printf("3 -- x1 unordered_map is now empty\n");
	if(x2.empty())
		printf("3 -- x2 unordered_map is now empty\n");
	if(x3.empty())
		printf("3 -- x3 unordered_map is now empty\n");



	//4.size_type size();															//返回unordered_map 的size()
	printf("4 -- x2 unordered_map size()=%d\n", x2.size());



	//5.顺向迭代器
	//iterator begin();																//返回unordered_map 的顺向front() 迭代器
	//iterator end();																	//返回unordered_map 的顺向end() 迭代器
	printf("5 -- x2:");
	...
	hash_map_pair_val = x2.insert(pair<int,int>(0,10));												//插入方式2: pair() + insert()
	hash_map_pair_val = x2.insert(pair<int,int>(1,20));
	hash_map_pair_val = x2.insert(pair<int,int>(2,30));
	hash_map_pair_val = x2.insert(pair<int,int>(3,40));
	hash_map_pair_val = x2.insert(pair<int,int>(4,50));

	x2[5] = 60;																																//插入方式3: '模仿'数组插入语法方式
	x2[6] = 70;
	//x2["name"] = "adan";														//假如是字符串, 可以这样操作

	hash_map_pair_val = x2.insert(unordered_map<int, int>::value_type(7, 80));//插入方式4: value_type() + insert()
	hash_map_pair_val = x2.insert(unordered_map<int, int>::value_type(8, 90));

	hash_map_pair_val = x2.emplace(pair<int,int>(99,1230));										//插入方式5: emplace 返回判断结果, 查看是否插入, 还是unordered_map 中已存在该值
	printf("%d, %d\n", *hash_map_pair_val.first, hash_map_pair_val.second);
	...
	//9.void swap(unordered_map<Key,Compare,Allocator>& st);									//unordered_map 之间交换元素
	x1.swap(x2);
