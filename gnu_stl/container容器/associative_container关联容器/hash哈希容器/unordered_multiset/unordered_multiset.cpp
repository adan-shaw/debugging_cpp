//编译:
//		g++ -std=c++11 -g3 ./unordered_multiset.cpp -o x 



//unordered_multiset api list
/*
	成员函数
	...
	非成员函数
		operator==				比较 unordered_multiset 中的值
		operator!=
		std::swap(std::unordered_multiset)	特化 std::swap 算法(C++11)
		erase_if(std::unordered_multiset)		擦除所有满足特定判别标准的元素(C++20)
*/
...
#include <cstdio>
#include <cassert>
#include <unordered_set>//unordered_multiset 同样定义在<unordered_set> 中
//#include<functional>
...
int main(void){
	...
	unordered_multiset<int>::key_equal eq;							//key 值比较函数
																											//pair一对变量
	pair<unordered_multiset<int>::iterator, unordered_multiset<int>::iterator> hash_multiset_pair_range;
	...
	//2.void clear();																		//清空unordered_multiset
	x1.clear();
	x3.clear();



	//3.bool empty();																		//检查unordered_multiset 是否为空
	if(x1.empty())
		printf("3 -- x1 unordered_multiset is now empty\n");
	if(x2.empty())
		printf("3 -- x2 unordered_multiset is now empty\n");
	if(x3.empty())
		printf("3 -- x3 unordered_multiset is now empty\n");



	//4.size_type size();																//返回unordered_multiset 的size()
	printf("4 -- x2 unordered_multiset size()=%d\n", x2.size());



	//5.顺向迭代器
	//iterator begin();																	//返回unordered_multiset 的顺向front() 迭代器
	//iterator end();																		//返回unordered_multiset 的顺向end() 迭代器
	printf("5 -- x2:");
	for(it = x2.begin(); it != x2.end(); *it++)
		printf("%d  ", *it);
	printf("\n");
	...
	//it = x2.insert(pair<unordered_multiset<int>::iterator,bool>(123));	//失败, 不能像map 那样用, 不能直接插入pair 对;
	...
	//9.void swap(unordered_multiset<Key,Compare,Allocator>& st);		//两个unordered_multiset之间交换元素
	x1.swap(x2);
	...
	/*
	hash_multiset_pair_range = x1.equal_range(500);														//超出范围的值搜索, 会直接等于end(), 表示被搜索的值不存在
	assert(hash_multiset_pair_range.first == x1.end());

	hash_multiset_pair_range = x1.equal_range(223);														//刚好搜索到最后一个值, hash_multiset_pair_range.second == x1.end()
	assert(hash_multiset_pair_range.second == x1.end());

	assert(hash_multiset_pair_range.first != hash_multiset_pair_range.second);//hash_multiset_pair_range.first 永远不等于 hash_multiset_pair_range.second 
	*/
	hash_multiset_pair_range = x1.equal_range(20);														//在key 唯一的容器中, 找不到会返回'范围区域', 或者end()迭代器; 在ket 不是唯一的场合, 用得比较多;
	if(hash_multiset_pair_range.first != x1.end())
		for(; hash_multiset_pair_range.first != hash_multiset_pair_range.second; hash_multiset_pair_range.first++) 
			printf("13 -- x1.equal_range(20)=%d\n", *hash_multiset_pair_range.first);
