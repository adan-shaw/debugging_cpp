...
/*
	...
	其实'线程key'就是func线程函数里面的局部变量, 设置成'线程key', 只是方便后续编码时索引而已;
	如果传递变量指针到函数, 也要压栈, 清栈, 而且十分不方便;
	调用pthread_getspecific() 获取线程私有全局变量, 实际上性能损耗不大, 但带来巨大的便利性, 划算;

	直接定义为local 局部变量, 当线程调用其他函数的时候, 又需要引用某些资源的时候,
	如果没有'线程key'私有全局变量, 会变得十分不好索引, 需要传递指针到函数参数;
*/
