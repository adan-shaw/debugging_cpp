...
int main(void){
	...
	for(j = 1; ; j++){
		...
		case -1:
			perror("fork()");
			printf("at last, j = %d\n",j);//一般情况下, 只有父进程会出错
																		//(最后一次调用fork()越界limit 限制)

			exit(EXIT_FAILURE);						//fork循环创建失败,退出程序
			// NOT REACHED //
		case 0:
			//sleep(10);//卡死子进程(不管用, 不需要卡死子进程)
									//系统限制最低为10462 个进程
									//子进程的尸体, 仍然占用'一个限制数', 仍然有效
									//*** unknow bug !! ***
			_exit(EXIT_SUCCESS);					//子进程留下尸体, 仍然会占用'一个限制数'
		default:
...
/*
	...
	resource可以指定的'变量宏':
		...
		RLIMIT_RTPRIO	进程可通过sched_setscheduler()
									和sched_setparam()设置的最大实时优先级.
		RLIMIT_NOFILE	指定比进程可打开的最大文件描述词大一的值, 超出此值,
									将会产生EMFILE错误.
