...
/*
	...
	F_DUPFD 返回一个如下描述的(文件)描述符:
		...
		(2)与原始操作符一样的某对象的引用
		(3)如果对象是file文件的话, 返回一个新的描述符, 这个描述符与arg共享相同的偏移量(offset)
		(4)相同的访问模式(读,写,读写)
		(5)相同的文件状态标志(如:两个文件描述符共享相同的状态标志)
		(6)与新的文件描述符结合在一起的close-on-exec标志, 被设置成交叉式访问execve(2)的系统调用
...
//一般不需要if 判断fcntl() 的返回结果, 以便节约if 的使用, 提高程序效率(只要open()/fopen() 打开了文件, fcntl()就基本不会出错, 懂的都懂)
...
//关于close-on-exec标志声明(在fork()/exec() 函数中会涉及):
/*
	在fork()/exec() 函数中会涉及close-on-exec 标志的使用, 被设置为close-on-exec=false 的fd 将不会被fork()/exec() 的子进程拷贝;

	fork()是读时共享, 写时拷贝(写时cp, 写时dup);
	言外之意就是, 如果你不写, fd 永远都一直只有一份!!
	fork()之后无需操作任何fd, 就不会被触发dup() 拷贝FILE 文件流;

	close-on-exec 标志是给exec() 新建全新进程系列函数使用的,
	如果创建一个全新的进程, 会涉及FILE 文件流资源的争夺, 则还是设置close-on-exec=false比较好.

	如果你自己主动dup() 拷贝了一个FILE 文件流, 你就要自己手动close()关闭, 或者进程结束的时候, 也会强制close() 所有文件流, 回收资源;
	...
	F_GETFD 取得与文件描述符fd联合close-on-exec标志, 类似FD_CLOEXEC.
					如果返回值和FD_CLOEXEC进行'与运算'的结果是0, 则文件保持交叉式访问exec();
					否则如果通过exec()运行的话, 文件将被关闭(arg被忽略)
...
/*
	...
	*F_DUPFD_CLOEXEC - Duplicate file descriptor with the close-on-exec flag FD_CLOEXEC set.
	...
	O_EXCL			- Exclusive use flag. [O_CREAT和O_EXCL同时设置, 当文件已存在时, open()失败, 保证不破坏已存在的文件]
	...
	O_DSYNC			- Write according to synchronized I/O data integrity completion. [同步到disk-写]
	O_RSYNC			- Synchronized read I/O operations. [同步到disk-读]
	O_SYNC			- Write according to synchronized I/O file integrity completion. [同步读写模式]
	...
	O_SEARCH		- Open directory for search only.[允许可以搜索, 仅文件夹有效]
