...
void std_stream_write(void){
	...
	//pfs = fopen("./test_data", "w");		//以默认模式-自动匹配,打开文件,写入应用数据
	pfs = fopen("./test_data", "wb");		//以binary模式打开, 写入应用数据
	//pfs = fopen("./test_data", "wt");	//以text 模式打开, 写入应用数据
	if(pfs == NULL){
		perror("fopen()");
		return;
	}

	fwrite(&x,sizeof(char),sizeof(x),pfs);
	fwrite(&x2,sizeof(char),sizeof(x2),pfs);
	fflush(pfs);
...
void std_stream_read(void){
	...
	//pfs = fopen("./test_data", "r");		//以默认模式-自动匹配,打开文件,读取应用数据
	pfs = fopen("./test_data", "rb");		//以binary模式打开, 读取应用数据
	//pfs = fopen("./test_data", "rt");	//以text 模式打开, 读取应用数据
	if(pfs == NULL){
		perror("fopen()");
		return;
	}
	...
	fread(&x,sizeof(char),sizeof(x),pfs);
	printf("x.a = %d\n",x.a);
	printf("x.b = %d\n",x.b);
	printf("x.c = %lf\n",x.c);
	printf("x.d = %f\n",x.d);
	...
	fread(&x2,sizeof(char),sizeof(x2),pfs);
	printf("x2.a = %d\n",x2.a);
	printf("x2.b = %d\n",x2.b);
	printf("x2.c = %lf\n",x2.c);
