...
/*
	...
	延伸阅读:
		...
		soft limit是指内核所能支持的资源上限.
			比如对于RLIMIT_NOFILE(一个进程能打开的最大文件数, 内核默认是1024), soft limit最大也只能达到1024.
	...
	setrlimit() 与getrlimit() 的使用细节:
		需要注意的是你在setrlimit(), 需要检查是否成功来判断新值有没有超过hard limit, 如:
			Linux系统中在应用程序运行过程中经常会遇到程序突然崩溃, 提示: Segmentation fault;
			这是因为应用程序收到了SIGSEGV信号, 这个信号提示: 当前进程发生了无效的存储访问;
		当接收到这个信号时, 缺省动作是: 终止w/core.
		终止w/core的含义是:
			在进程当前目录生成core文件, 并将进程的内存映象复制到core文件中,
			core文件的默认名称就是"core"(这是 Unix类系统的一个由来已久的功能).

		事实上, 并不是只有SIGSEGV信号产生coredump, 还有下面一些信号也产生coredump:
