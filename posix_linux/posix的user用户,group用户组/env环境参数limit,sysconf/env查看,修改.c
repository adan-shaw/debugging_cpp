...
//env '进程环境变量'查看&修改, 简介:
/*
	linux 默认情况下, 命令行'所有'进程环境变量'字符串'的数据总长, 应小于128kb;
	putenv()在环境中设置环境变量的string 总长, max 限制似乎约为128kb;
...
int main(void){
	...
	strncpy(env_buf,"ADAN_TEST=\"come_love_me\"",sizeof(env_buf));
	tmp = putenv(env_buf);
	if(tmp != 0){
	...
	*/
	strncpy(env_buf,"ADAN_TEST",sizeof(env_buf));
	//覆盖式修改
	tmp = setenv(env_buf, "\"setenv()_love_me\"", 1);
	if(tmp == -1){
		perror("setenv()");
		return -1;
	}
