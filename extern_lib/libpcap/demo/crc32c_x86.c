...
/* Multiply a matrix times a vector over the Galois field of two elements, GF(2).
   ...
   least as many entries as the power of two for most significant one bit in vec. */
...
/* Construct an operator to apply len zeros to a crc.  len must be a power of two.
   If len is not a power of two, then the result is the same as for the largest power of two less than len.
   The result for len == 0 is the same as for len == 1.
