...
static void DoS_icmp(void){
	...
	memcpy(&iph->ip_src, &tmp, sizeof(struct sockaddr_in));
	//发送目标地址
	memcpy(&iph->ip_dst, &dest, sizeof(struct sockaddr_in));
	...
	memcpy(&to.sin_addr.s_addr, &iph->ip_dst, sizeof(struct sockaddr_in));
	to.sin_port = htons(0);
