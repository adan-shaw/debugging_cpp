...
static void DoS_udp(void){
	...
	dosseg.iph.ip_sum = DoS_cksum ((uint16_t *) & dosseg.iph, sizeof (dosseg.iph));
	...
	//校验和
	dosseg.udph.uh_sum = DoS_cksum((uint16_t*)&dosseg.udph, tot_len);
#else
	//UDP 源端口
	...
	dosseg.udph.check = 0;
	//校验和
	dosseg.udph.check = DoS_cksum((uint16_t*)&dosseg.udph, tot_len);
#endif

	//填写发送目的地址部分
	to.sin_family = AF_INET;
	memcpy(&to.sin_addr.s_addr, &dest, sizeof(struct sockaddr_in));
	to.sin_port = htons(0);
