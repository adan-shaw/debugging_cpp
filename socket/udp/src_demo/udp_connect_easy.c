...
//udp 在实施过程中遇到的问题(验证ok!!):
/*
	最简单的方式(udp_connect_easy.c):
...
//udp 在实施过程中遇到的问题(猜想, 未实现, 不可靠, 没有实现的必要!!):
/*
	* udp client:
	...
	if(tmp == 0){//子进程: 客户端
		...
		*/
		...
		if(tmp == -1){
			...
			sfd_tmp = dup(sfd);//直接拷贝sfd, 共用addr + port
		...
		wait(&tmp);//等待子进程结束
		printf("[server] son process quit, return %d\n", tmp);
	}
