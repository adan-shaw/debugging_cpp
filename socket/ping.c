...
//查找一个合适的包位置: 当seq为-1时, 表示查找空包; 其他值表示查找seq对应的包
static ping_packet* icmp_findpacket(int seq){
	int i = 0;
	...
	if(seq == -1){//查找空的位置
		for(i = 0;i<128;i++){
			if(m_ping_packet[i].flag == 0){
				found = &m_ping_packet[i];
				break;
			}
		}
	}
...
int main(int argc, char*argv[]){
	...
	memset(m_ping_packet, 0, sizeof(ping_packet) * 128);
