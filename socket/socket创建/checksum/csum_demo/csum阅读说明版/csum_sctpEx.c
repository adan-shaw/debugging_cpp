...
uint16_t checksum_sctp (const void *buf, size_t len, uint32_t saddr, uint32_t daddr)
{
	...
	// If the length is odd, add the padding byte
	if (len % 2 != 0)
	{
	...
	sum += 0x0000FFFF;						// Protocol number for SCTP is 132
	if (sum > 0xFFFF)
	{
		sum = (sum & 0xFFFF) + (sum >> 16);
	}
