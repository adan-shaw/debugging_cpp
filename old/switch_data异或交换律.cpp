...
*/
void swap4b_N(unsigned int *pa, unsigned int *pb, unsigned int N){
...
void sw_1byteN(unsigned char *pa, unsigned char *pb, unsigned int N){
	for(; N>0; N--,pa++,pb++){
		*pa = (*pa)^(*pb);
		*pb = (*pa)^(*pb);
		*pa = (*pa)^(*pb);
...
void test_swap4b_N(void){
	...
	printf("s1=%d,s2=%d\n",s1,s2);
	swap4b_N((unsigned int*)&s1, (unsigned int*)&s2 ,1);
	printf("s1=%d,s2=%d\n",s1,s2);
	printf("wrong!!\n");
	...
	printf("sizeof(unsigned int)=%d\n",sizeof(unsigned int));
	printf("i1=%d,i2=%d\n",i1,i2);
	swap4b_N((unsigned int*)&i1, (unsigned int*)&i2 ,1);
	printf("i1=%d,i2=%d\n",i1,i2);

	//交换32bit float(4 字节)
	printf("sizeof(float)=%d\n",sizeof(float));
	printf("f1=%f,f2=%f\n",f1,f2);
	swap4b_N((unsigned int*)&f1, (unsigned int*)&f2, 1);
	printf("f1=%f,f2=%f\n",f1,f2);
	...
	printf("d1=%lf,d2=%lf\n",d1,d2);
	swap4b_N((unsigned int*)&d1, (unsigned int*)&d2 ,2);
	printf("d1=%lf,d2=%lf\n",d1,d2);
	...
	printf("c1=%s,c2=%s\n",c1,c2);
	swap4b_N((unsigned int*)&c1, (unsigned int*)&c2 ,2);
	printf("c1=%s,c2=%s\n",c1,c2);

	//交换128bit-char(16 字节)
	printf("c3=%s,c4=%s\n",c3,c4);
	swap4b_N((unsigned int*)&c3, (unsigned int*)&c4 ,4);
	printf("c3=%s,c4=%s\n",c3,c4);

	//交换96bit-char(12 字节, 4*3 也可以!!)
	printf("c5=%s,c6=%s\n",c5,c6);
	swap4b_N((unsigned int*)&c5, (unsigned int*)&c6 ,3);
	printf("c5=%s,c6=%s\n",c5,c6);

	//交换88bit-char(11 字节, 4*2+3, 就会出错!! 必须为4字节*N, 因为每次unsigned int* 后移4字节)
	printf("c55=%s,c66=%s\n",c55,c66);
	swap4b_N((unsigned int*)&c55, (unsigned int*)&c66 ,3);
	printf("c55=%s,c66=%s\n",c55,c66);

	//交换32bit-char(4 字节)
	printf("c7=%s,c8=%s\n",c7,c8);
	swap4b_N((unsigned int*)&c7, (unsigned int*)&c8, 1);
	printf("c7=%s,c8=%s\n",c7,c8);

	//交换256bit-char(32 字节)
	printf("c9=%s,c0=%s\n",c9,c0);
	swap4b_N((unsigned int*)&c9, (unsigned int*)&c0 ,8);
	printf("c9=%s,c0=%s\n",c9,c0);
