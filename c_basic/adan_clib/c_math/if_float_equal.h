...
//比较两个double, 相等返回true=1, 不相等返回false=0
/*
	c++ double/float 浮点, 
	...
	1e-32 = 小数点后32个0
*/
#define if_double_equal(x, y)  ( fabs(x-y)<(1e-8)  ? 1 : 0 )
#define if_float_equal(x, y)   ( fabsf(x-y)<(1e-8) ? 1 : 0 )
#define if_ldouble_equal(x, y) ( fabsl(x-y)<(1e-8) ? 1 : 0 )
...
/*
	...
	if(tmp > 0){															//select(): 有io 事件(故意将最优选项前置, 提高预读性能)
